...
final class PoseDetector {
    ...
    func detect(_ sampleBuffer: CMSampleBuffer, timestamp: TimeInterval) throws -> PoseFrame {
        ...
        guard let pose = result.landmarks.first else { return PoseFrame(landmarks: [], confidence: 0, kneeAngle: nil, imageAspectRatio: aspectRatio) }
        ...
        return PoseFrame(landmarks: points, confidence: chosen?.confidence ?? 0, kneeAngle: chosen?.angle, imageAspectRatio: aspectRatio)
    }
